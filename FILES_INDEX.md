...
ops/dashboard.py | CORE (Phase 17) | FastAPI single-page staged control center with live candlestick chart, stage action buttons (promote/demote/kill/revive), governance health bar, stage transition history timeline, graveyard for killed pairs, QA learning insights panel, fleet-wide pause/resume controls. Ordered as: system health -> chart -> governance -> stage history -> prod -> QA -> watchers -> graveyard -> signal feed | manual,autostart | fastapi,uvicorn,argus_flow logs | NO | broker_snapshot,alert_state,evidence_registry,deployment_registry,signals,trades,heartbeat | none (HTTP only) | ops,monitoring,qa-visibility
argus_flow/advanced_features.py | CORE | Multi-timeframe bar aggregation (5m/15m/30m/1h/4h), spread gate, adaptive session scoring, news/economic calendar filter, conviction scoring, correlation-aware entry sequencing | runner_unified.py | numpy | NO | runtime tick data | features stamped on signals+trades | strategy,features
argus_flow/ops/stage_actions.py | OPS CORE | Execute manual and automatic stage transitions, persist history to stage_history.jsonl. Supports: promote, demote, quarantine, kill, revive, pause, unpause | dashboard POST /api/stage_action, managed truth refresh | deployment_pipeline | NO | deployment_registry, configs | stage_history.jsonl | ops,staging,governance
argus_flow/ops/deploy_variants.py | OPS | Generate aggressive/conservative watcher variant configs per FX pair for parallel A/B testing | manual | json | NO | configs | variant configs in configs/ | strategy,variants,research
argus_flow/ops/qa_learning.py | OPS ANALYTICS | Trade analytics: regime performance, session heatmap, exit quality, direction analysis, duration buckets, streak analysis, variant comparison | managed truth refresh | csv | NO | trades.csv per runner | qa_learning_report.json | analytics,strategy
argus_flow/ops/trade_metrics.py | OPS ANALYTICS | Fleet-wide and per-instrument aggregate trade metrics: PnL, WR, PF, drawdown, rolling-20, entry hour distribution | manual | csv | NO | trades.csv per runner | trade_metrics.json | analytics,metrics
argus_flow/ops/fleet_correlation.py | OPS ANALYTICS | Pairwise daily PnL correlation matrix, PCA eigenvalue decomposition, effective independent bets, worst-day analysis | manual | numpy | NO | trades.csv per runner | fleet_correlation_report.json | analytics,risk,correlation
argus_flow/ops/param_sensitivity.py | OPS RESEARCH | Generate +/-N% perturbation configs for parameter sensitivity analysis | manual | json | NO | configs | sensitivity configs in configs/_sensitivity | research,sensitivity
argus_flow/ops/ablation_test.py | OPS RESEARCH | Generate ablation test configs disabling one trigger feature at a time to isolate signal vs noise | manual | json | NO | configs | ablation configs in configs/_ablation | research,ablation
argus_flow/ops/atr_stops.py | OPS RESEARCH | ATR-scaled stop/target analysis and config generator per instrument | manual | csv | NO | signals.csv | atr_stops_report.json | strategy,stops
argus_flow/ops/emergency_close.py | OPS | Standalone CLI to close specific or all IBKR positions via market orders, independent of runner process | manual | ib-insync | NO | IBKR connection | console output | ops,emergency
argus_flow/ops/log_rotation.py | OPS | Rotate signals.csv, trades.csv, opportunities.jsonl when they exceed configurable line threshold. Archives to gzip | manual,scheduler | gzip | NO | per-runner log dirs | archive/*.gz | ops,maintenance
argus_flow/ops/edge_allocation.py | OPS ANALYTICS | Edge-weighted capital allocation framework: scores pairs by PF/WR/confidence, outputs allocation multiplier. LOG_ONLY until 20+ trades per pair | managed truth refresh | csv | NO | trades.csv per runner | edge_allocation_report.json | analytics,allocation,risk
argus_flow/tests/test_orphan_recovery.py | TEST | 6-scenario integration test for orphan order recovery: orphan, phantom, clean, matched, untracked, broker unavailable | manual | runner_unified | NO | temp sandbox | console PASS/FAIL | testing,recovery
requirements.lock | CONFIG | Full pip freeze (83 packages) for reproducible installs across machines | manual | pip | NO | venv | none | dependencies,reproducibility
