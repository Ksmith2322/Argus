...
# MTF features (v5 schema — from MTF engine + AI overlay)
MTF_NUMERIC_FEATURES = [
    "mtf_confidence",
    "mtf_rsi_1h",
    "ai_consensus",
    "ai_confidence",
...
MTF_CATEGORICAL_FEATURES = [
    "mtf_trend_4h",  # UP / DOWN
    "mtf_setup_1h",  # PULLBACK_TO_EMA / BREAK_OF_STRUCTURE / TREND_CONTINUATION
    "ai_action",     # TAKE / SKIP
]
...
ALL_NUMERIC = SIGNAL_NUMERIC_FEATURES + MTF_NUMERIC_FEATURES + DERIVED_NUMERIC_FEATURES
ALL_FEATURES = ALL_NUMERIC + CATEGORICAL_FEATURES + MTF_CATEGORICAL_FEATURES
    ...
    for symbol in FX_PAIRS:
        ...
        for t in trades:
            ...
            record = {
                ...
                # MTF numeric features (v5 schema)
                "mtf_confidence": _safe_float(signal.get("mtf_confidence", "")),
                "mtf_rsi_1h": _safe_float(signal.get("mtf_rsi_1h", signal.get("mtf_rsi", ""))),
                "ai_consensus": _safe_float(signal.get("ai_consensus", "")),
                "ai_confidence": _safe_float(signal.get("ai_confidence", "")),
                "trend_strength": _safe_float(signal.get("trend_strength", "")),
                "efficiency_ratio": _safe_float(signal.get("efficiency_ratio", "")),
                # Derived
                "hour_sin": hour_sin,
                "hour_cos": hour_cos,
                # Categorical
                ...
                "symbol": symbol.upper(),
                # MTF categorical
                "mtf_trend_4h": signal.get("mtf_trend_4h", "unknown") or "unknown",
                "mtf_setup_1h": signal.get("mtf_setup_1h", "unknown") or "unknown",
                "ai_action": signal.get("ai_action", "unknown") or "unknown",
                # Label
...
def collect_backtest_trades(bt_paths: list[Path]):
    """Load trades from backtest training CSVs (generated by fx_backtest --mtf)."""
    records = []
    for path in bt_paths:
        ...
        for t in rows:
            ...
            hour = _safe_float(t.get("hour", ""))
            hour_sin = math.sin(2 * math.pi * hour / 24) if not np.isnan(hour) else np.nan
            hour_cos = math.cos(2 * math.pi * hour / 24) if not np.isnan(hour) else np.nan
            epoch = _parse_ts(t.get("ts", ""))
            record = {
                "range_pct": _safe_float(t.get("range_pct", "")),
                "vol_z": _safe_float(t.get("vol_z", "")),
                "range_accel": _safe_float(t.get("range_accel", "")),
                "dist_from_low": _safe_float(t.get("dist_from_low", "")),
                "mtf_confidence": _safe_float(t.get("mtf_confidence", "")),
                "mtf_rsi_1h": _safe_float(t.get("mtf_rsi_1h", "")),
                "ai_consensus": _safe_float(t.get("ai_consensus", "")),
                "ai_confidence": _safe_float(t.get("ai_confidence", "")),
                "trend_strength": _safe_float(t.get("trend_strength", "")),
                "efficiency_ratio": _safe_float(t.get("efficiency_ratio", "")),
                "hour_sin": hour_sin,
                "hour_cos": hour_cos,
                "direction": t.get("direction", "unknown").strip().lower() or "unknown",
                "symbol": path.stem.split("_")[0].upper(),
                "mtf_trend_4h": t.get("mtf_trend_4h", "unknown") or "unknown",
                "mtf_setup_1h": t.get("mtf_setup_1h", "unknown") or "unknown",
                "ai_action": t.get("ai_action", "unknown") or "unknown",
                "pnl_pips": pnl,
            ...
            records.append(record)
        print(f"  Loaded {len(rows)} trades from {path.name}")
    return records
...
def build_dataset(records):
    ...
    all_cat_features = CATEGORICAL_FEATURES + MTF_CATEGORICAL_FEATURES
    ...
    for rec in records:
        row_numeric = [rec.get(f, np.nan) for f in ALL_NUMERIC]
        row_cat = [rec.get(f, "unknown") for f in all_cat_features]
        X_rows.append(row_numeric + row_cat)
        y.append(rec["win"])
        epochs.append(rec["_epoch"])
        pnls.append(rec.get("pnl_pips", 0) or 0)
    ...
    # Encode categoricals
    encoders = {}
    X_cat = np.zeros((len(X_rows), len(all_cat_features)), dtype=float)
    for j, f in enumerate(all_cat_features):
        le = LabelEncoder()
        vals = [str(row[n_num + j]) for row in X_rows]
...
def main():
    ...
    parser.add_argument("--backtest-data", nargs="+",
                        help="Additional backtest training CSVs (from fx_backtest --mtf)")
    ...
    # 1b. Load backtest training data if provided
    if args.backtest_data:
        print(f"\nLoading backtest training data...")
        bt_records = collect_backtest_trades([Path(p) for p in args.backtest_data])
        records.extend(bt_records)
        stats["matched"] += len(bt_records)
        print(f"  Added {len(bt_records)} backtest trades (total: {stats['matched']})")
