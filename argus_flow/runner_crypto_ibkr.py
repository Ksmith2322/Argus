"""IBKR Crypto Paper Trading Runner (BTC/ETH via Paxos).

Uses the same range_accel strategy but adapted for 24/7 crypto markets.

Usage:
    python -m argus_flow.runner_crypto_ibkr --symbol BTC --config argus_flow/configs/btc_range_paper_v1.json --client-id 25
"""
...
log = logging.getLogger("crypto_ibkr")
...
class State:
    ...
    def save(self):
        self.file.write_text(json.dumps({
            ...
            "trade_count": self.trade_count, "pnl_pips": self.pnl_usd,
        }, indent=2))
...
def main(symbol, config_path, client_id):
    cfg = json.loads(Path(config_path).read_text())
    risk = cfg.get("risk", {})
    stop_bps = risk.get("stop_bps", 50)
    target_bps = risk.get("target_bps", 100)
    timeout_min = risk.get("timeout_minutes", 60)
    min_gap = risk.get("min_signal_gap_minutes", 15)

    log_dir = Path(f"argus_flow/logs/{symbol.lower()}_ibkr")
    log_dir.mkdir(parents=True, exist_ok=True)
    trade_log = log_dir / "trades.csv"
    signal_log = log_dir / "signals.csv"

    log.info("=" * 60)
    log.info(f"{symbol} Crypto Runner (IBKR Paxos)")
    ...
    if not qualified:
        log.error(f"Could not qualify {symbol} crypto contract")
        ib.disconnect()
        return True
    ...
    # Crypto may not have historical data on IBKR paper — seed with empty buffer
    log.info("Loading recent bars...")
    try:
        bars = ib.reqHistoricalData(contract, endDateTime="", durationStr="2 D",
                                     barSizeSetting="1 min", whatToShow="AGGTRADES", useRTH=False, timeout=15)
        for b in bars:
            buf.add({"ts": str(b.date), "open": b.open, "high": b.high,
                     "low": b.low, "close": b.close, "volume": b.volume})
        log.info(f"Seeded {len(buf)} bars")
    except Exception as e:
        log.warning(f"No historical data for {symbol}: {e}. Starting with empty buffer.")

    current_bar_minute = None
    current_bar = {"open": 0, "high": 0, "low": 999999, "close": 0, "volume": 0}
    ...
    try:
        while True:
            ...
            if state.position != "FLAT":
                ...
                if exit_reason:
                    if state.position == "LONG":
                        pnl = mid - state.entry_price
                    else:
                        pnl = state.entry_price - mid
                    state.pnl_usd += pnl
                    ...
                    with open(trade_log, "a", newline="") as f:
                        csv.writer(f).writerow([now.isoformat(), state.position.lower(), f"{state.entry_price:.2f}", f"{mid:.2f}", f"{pnl:.2f}", exit_reason, f"{dur:.1f}", state.trade_count])

                    log.info(f"EXIT {state.position} @ {mid:.2f} reason={exit_reason} pnl=${pnl:+.2f} total=${state.pnl_usd:+.2f} trades={state.trade_count}")
                    state.position = "FLAT"
                    state.save()
                continue
            ...
            if len(buf) >= 60:
                ...
                if direction:
                    ...
                    log.info(f"ENTRY {direction.upper()} @ {entry_px:.2f} stop={stop_px:.2f} target={target_px:.2f}")
                else:
...
def run():
    ...
    for attempt in range(max_retries):
        ...
        should_reconnect = main(args.symbol, args.config, args.client_id)
