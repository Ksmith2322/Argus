"""forge.hyg_apr_hold -- HYG April (high-yield credit) seasonal, live runner.

Calendar-based runner. Wakes daily; on the first weekday of
April, BUYS HYG at close. On the last weekday of April, SELLS at
close. One trade per year.

Backtest: 19y on HYG, n=20, PF=7.41, CI lower 1.71, WR=85%,
avg +1.74% per trade. From the 2026-05-26 extension matrix sweep
(ops/reports/system_audit/extension_matrix_sweep.md). High-yield
credit April seasonal -- post-Q1 risk-on rotation + earnings-season
issuance window. Orthogonal to the equity + Treasury + gold calendar
strategies in the existing fleet (HYG behaves more like equity than
like IEF, but the seasonal driver differs).
...
plumbing as forge.nov_spy.runner -- calendar-aware market orders.
"""
...
STRATEGY_LABEL = "forge_hyg_apr_hold"
IBKR_CLIENT_ID = 136
_SIGNAL_ONLY_MODE = False

REPO = Path(__file__).resolve().parents[2]
LOG_DIR = REPO / "forge" / "logs" / "hyg_apr_hold"
...
log = setup_logging("hyg_apr_hold")
...
PARAMS = {
    ...
    "eval_hour_utc": 19,
    "eval_minute_utc": 53,         # offset to spread broker load across the calendar pack
}
...
def evaluate_once(force_action: str | None = None) -> dict:
    ...
    if not (is_entry or is_exit):
        summary["reason"] = (f"not a hyg_apr action day "
    ...
    if _SIGNAL_ONLY_MODE:
        ...
        log.info("SIGNAL-ONLY: would %s HYG", action_word)
...
def loop_mode() -> None:
    log.info("hyg_apr_hold --loop started: daily wake at %02d:%02d UTC",
    ...
    while True:
        try:
            ...
            log.info("hyg_apr_hold next wake at %s UTC (sleep %.0fs)",
            ...
            log.info("hyg_apr_hold loop stopped by user")
            return
