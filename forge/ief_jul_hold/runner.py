"""forge.ief_jul_hold -- IEF July effect, live runner.

Calendar-based runner. Wakes daily; on the first weekday of
July, BUYS IEF at close. On the last weekday of July, SELLS at
close. One trade per year.

Backtest: 20y on IEF, n=20, PF=12.14, CI lower 4.21, WR=75%,
avg +1.0% per trade. Cleanest survivor in the 2026-05-26 extension
matrix sweep (ops/reports/system_audit/extension_matrix_sweep.md).
Bonds rally in July is a known Treasury-auction / summer flight-to-
quality seasonal -- orthogonal to every equity strategy we run.
...
STRATEGY_LABEL = "forge_ief_jul_hold"
IBKR_CLIENT_ID = 133
_SIGNAL_ONLY_MODE = False

REPO = Path(__file__).resolve().parents[2]
LOG_DIR = REPO / "forge" / "logs" / "ief_jul_hold"
...
log = setup_logging("ief_jul_hold")
...
PARAMS = {
    ...
    "eval_hour_utc": 19,
    "eval_minute_utc": 45,         # offset from nov_spy/tom_spy to spread broker load
}
...
def evaluate_once(force_action: str | None = None) -> dict:
    ...
    if not (is_entry or is_exit):
        summary["reason"] = (f"not an ief_jul action day "
    ...
    if _SIGNAL_ONLY_MODE:
        ...
        log.info("SIGNAL-ONLY: would %s IEF", action_word)
...
def loop_mode() -> None:
    log.info("ief_jul_hold --loop started: daily wake at %02d:%02d UTC",
    ...
    while True:
        try:
            ...
            log.info("ief_jul_hold next wake at %s UTC (sleep %.0fs)",
            ...
            log.info("ief_jul_hold loop stopped by user")
            return
