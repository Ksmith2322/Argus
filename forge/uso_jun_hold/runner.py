"""forge.uso_jun_hold -- USO June (crude oil) seasonal, live runner.

Calendar-based runner. Wakes daily; on the first weekday of
June, BUYS USO at close. On the last weekday of June, SELLS at
close. One trade per year.

Backtest: 20y on USO, n=20, PF=4.07, CI lower 1.49, WR=75%,
avg +3.32% per trade. From the 2026-05-26 extension matrix sweep
(ops/reports/system_audit/extension_matrix_sweep.md). Crude oil
June seasonal -- driving season demand inflection, OPEC mid-year
review window, hurricane-season risk premium build. Orthogonal to
the equity + bond + gold calendar strategies in the existing fleet.
...
STRATEGY_LABEL = "forge_uso_jun_hold"
IBKR_CLIENT_ID = 135
_SIGNAL_ONLY_MODE = False

REPO = Path(__file__).resolve().parents[2]
LOG_DIR = REPO / "forge" / "logs" / "uso_jun_hold"
...
log = setup_logging("uso_jun_hold")
...
PARAMS = {
    ...
    "eval_hour_utc": 19,
    "eval_minute_utc": 55,         # offset to spread broker load across the calendar pack
}
...
def evaluate_once(force_action: str | None = None) -> dict:
    ...
    if not (is_entry or is_exit):
        summary["reason"] = (f"not a uso_jun action day "
    ...
    if _SIGNAL_ONLY_MODE:
        ...
        log.info("SIGNAL-ONLY: would %s USO", action_word)
...
def loop_mode() -> None:
    log.info("uso_jun_hold --loop started: daily wake at %02d:%02d UTC",
    ...
    while True:
        try:
            ...
            log.info("uso_jun_hold next wake at %s UTC (sleep %.0fs)",
            ...
            log.info("uso_jun_hold loop stopped by user")
            return
