"""Calendar/event date sources for the backtest factory.

The factory's calendar entry kinds reference event sets by name. This
module is the single source of truth for those sets so specs stay
JSON-serializable (the spec just names the set, the engine looks up the
dates here).

Available sets:
    - "FOMC": ~200 announcement dates 2000-2026 (Fed monetary-policy
      meetings, source federalreserve.gov/fomccalendars). Lifted from
      forge/fomc_drift_backtest.py — single canonical list.
    - "OPEX": 3rd Friday of every month (options expiration). Computed
      on-the-fly; not a fixed list.

To add a new event set, append the dates here and reference by name.
Avoid runtime API calls — calendar data should be deterministic so
backtests are reproducible.
"""
from __future__ import annotations

from datetime import date
from typing import Iterable

import pandas as pd


# FOMC announcement dates 2000-2026. Synced from forge/fomc_drift_backtest.py.
FOMC_DATES_RAW = [
    "2000-02-02", "2000-03-21", "2000-05-16", "2000-06-28",
    "2000-08-22", "2000-10-03", "2000-11-15", "2000-12-19",
    "2001-01-03", "2001-01-31", "2001-03-20", "2001-04-18",
    "2001-05-15", "2001-06-27", "2001-08-21", "2001-09-17",
    "2001-10-02", "2001-11-06", "2001-12-11",
    "2002-01-30", "2002-03-19", "2002-05-07", "2002-06-26",
    "2002-08-13", "2002-09-24", "2002-11-06", "2002-12-10",
    "2003-01-29", "2003-03-18", "2003-05-06", "2003-06-25",
    "2003-08-12", "2003-09-16", "2003-10-28", "2003-12-09",
    "2004-01-28", "2004-03-16", "2004-05-04", "2004-06-30",
    "2004-08-10", "2004-09-21", "2004-11-10", "2004-12-14",
    "2005-02-02", "2005-03-22", "2005-05-03", "2005-06-30",
    "2005-08-09", "2005-09-20", "2005-11-01", "2005-12-13",
    "2006-01-31", "2006-03-28", "2006-05-10", "2006-06-29",
    "2006-08-08", "2006-09-20", "2006-10-25", "2006-12-12",
    "2007-01-31", "2007-03-21", "2007-05-09", "2007-06-28",
    "2007-08-07", "2007-09-18", "2007-10-31", "2007-12-11",
    "2008-01-22", "2008-01-30", "2008-03-18", "2008-04-30",
    "2008-06-25", "2008-08-05", "2008-09-16", "2008-10-08",
    "2008-10-29", "2008-12-16",
    "2009-01-28", "2009-03-18", "2009-04-29", "2009-06-24",
    "2009-08-12", "2009-09-23", "2009-11-04", "2009-12-16",
    "2010-01-27", "2010-03-16", "2010-04-28", "2010-06-23",
    "2010-08-10", "2010-09-21", "2010-11-03", "2010-12-14",
    "2011-01-26", "2011-03-15", "2011-04-27", "2011-06-22",
    "2011-08-09", "2011-09-21", "2011-11-02", "2011-12-13",
    "2012-01-25", "2012-03-13", "2012-04-25", "2012-06-20",
    "2012-08-01", "2012-09-13", "2012-10-24", "2012-12-12",
    "2013-01-30", "2013-03-20", "2013-05-01", "2013-06-19",
    "2013-07-31", "2013-09-18", "2013-10-30", "2013-12-18",
    "2014-01-29", "2014-03-19", "2014-04-30", "2014-06-18",
    "2014-07-30", "2014-09-17", "2014-10-29", "2014-12-17",
    "2015-01-28", "2015-03-18", "2015-04-29", "2015-06-17",
    "2015-07-29", "2015-09-17", "2015-10-28", "2015-12-16",
    "2016-01-27", "2016-03-16", "2016-04-27", "2016-06-15",
    "2016-07-27", "2016-09-21", "2016-11-02", "2016-12-14",
    "2017-02-01", "2017-03-15", "2017-05-03", "2017-06-14",
    "2017-07-26", "2017-09-20", "2017-11-01", "2017-12-13",
    "2018-01-31", "2018-03-21", "2018-05-02", "2018-06-13",
    "2018-08-01", "2018-09-26", "2018-11-08", "2018-12-19",
    "2019-01-30", "2019-03-20", "2019-05-01", "2019-06-19",
    "2019-07-31", "2019-09-18", "2019-10-30", "2019-12-11",
    "2020-01-29", "2020-03-03", "2020-03-15", "2020-04-29",
    "2020-06-10", "2020-07-29", "2020-09-16", "2020-11-05",
    "2020-12-16",
    "2021-01-27", "2021-03-17", "2021-04-28", "2021-06-16",
    "2021-07-28", "2021-09-22", "2021-11-03", "2021-12-15",
    "2022-01-26", "2022-03-16", "2022-05-04", "2022-06-15",
    "2022-07-27", "2022-09-21", "2022-11-02", "2022-12-14",
    "2023-02-01", "2023-03-22", "2023-05-03", "2023-06-14",
    "2023-07-26", "2023-09-20", "2023-11-01", "2023-12-13",
    "2024-01-31", "2024-03-20", "2024-05-01", "2024-06-12",
    "2024-07-31", "2024-09-18", "2024-11-07", "2024-12-18",
    "2025-01-29", "2025-03-19", "2025-05-07", "2025-06-18",
    "2025-07-30", "2025-09-17", "2025-10-29", "2025-12-17",
    "2026-01-28", "2026-03-18", "2026-04-29", "2026-06-17",
    "2026-07-29", "2026-09-16", "2026-10-28", "2026-12-16",
]


def _first_friday_of_month(year: int, month: int) -> pd.Timestamp:
    """Compute the first Friday of (year, month). Used for NFP releases —
    Bureau of Labor Statistics has released the Employment Situation Report
    on the first Friday of each month for decades."""
    d = pd.Timestamp(year=year, month=month, day=1)
    # weekday(): Mon=0 ... Fri=4 ... Sun=6
    offset_to_friday = (4 - d.weekday()) % 7
    return d + pd.Timedelta(days=offset_to_friday)


def _nfp_dates_in_range(start_year: int = 2006, end_year: int = 2027) -> list[pd.Timestamp]:
    """Generate NFP release dates (first Friday of each month) for the
    given year range. Covers the 20y window the factory backtests use."""
    dates = []
    for y in range(start_year, end_year + 1):
        for m in range(1, 13):
            dates.append(_first_friday_of_month(y, m))
    return dates


def get_event_dates(event_set: str) -> list[pd.Timestamp]:
    """Return the list of pd.Timestamps for the named event set."""
    if event_set.upper() == "FOMC":
        return [pd.Timestamp(d) for d in FOMC_DATES_RAW]
    if event_set.upper() == "NFP":
        return _nfp_dates_in_range()
    raise ValueError(f"unknown event_set: {event_set!r}. Known: FOMC, NFP")


def bars_until_next_event(
    index: pd.DatetimeIndex,
    event_dates: Iterable[pd.Timestamp],
) -> pd.Series:
    """For each bar in `index`, return the number of TRADING bars until
    the next event date. NaN if no event remaining.

    A bar AT an event date has value 0. Bars after the last event date
    are NaN. Used as a feature for "N days before FOMC" type entries.
    """
    if isinstance(index, pd.DatetimeIndex) and index.tz is not None:
        index = index.tz_localize(None)
    event_dates = sorted(pd.Timestamp(d) for d in event_dates)
    if not event_dates:
        return pd.Series([float("nan")] * len(index), index=index)
    out = []
    ei = 0  # pointer into event_dates
    # Map each event to its trading-bar position (or the next bar on/after it)
    event_positions: list[int] = []
    for ev in event_dates:
        # find first bar index >= ev
        pos = index.searchsorted(ev, side="left")
        if pos < len(index):
            event_positions.append(int(pos))
        else:
            event_positions.append(len(index))  # past end
    # For each bar i, find smallest event_position >= i
    ev_arr = sorted(set(event_positions))
    for i in range(len(index)):
        # binary search for next ev pos >= i
        lo, hi = 0, len(ev_arr)
        while lo < hi:
            mid = (lo + hi) // 2
            if ev_arr[mid] < i:
                lo = mid + 1
            else:
                hi = mid
        if lo >= len(ev_arr) or ev_arr[lo] >= len(index):
            out.append(float("nan"))
        else:
            out.append(float(ev_arr[lo] - i))
    return pd.Series(out, index=index)
