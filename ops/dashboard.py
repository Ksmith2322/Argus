...
async def api_fleet_equity_curve(window_days: int = 90, include_backfill: bool = False):
    ...
    # SPY benchmark series — same date range, % return from start. Lets the
    # dashboard render "fleet vs SPY" as overlaid lines without the operator
    # having to compute it manually. Per project_future_dashboard_ideas Idea #1.
    # NOT beta-adjusted; just buy-and-hold % delta from t0. Disabled if no
    # points (empty fleet or yfinance fetch fails).
    spy_points: list[dict] = []
    if points:
        try:
            t_start = datetime.fromisoformat(points[0]["ts"].replace("Z", "+00:00"))
            t_end = datetime.fromisoformat(points[-1]["ts"].replace("Z", "+00:00"))
            span_days = max(1, int((t_end - t_start).total_seconds() / 86400) + 5)
            import warnings
            warnings.simplefilter("ignore")
            import yfinance as yf
            df = yf.download("SPY", period=f"{span_days}d", interval="1d",
                             progress=False, auto_adjust=False)
            if not df.empty:
                # yfinance newer versions return MultiIndex columns, so df["Close"]
                # is a DataFrame, not Series. Squeeze to a single Series before
                # iterating — `.items()` on a DataFrame iterates COLUMNS, not rows.
                close = df["Close"]
                if hasattr(close, "ndim") and close.ndim == 2:
                    close = close.iloc[:, 0]  # take first (and only) column as Series
                baseline = float(close.iloc[0])
                if baseline > 0:
                    for ts, val in close.items():
                        v = float(val)
                        bar_dt = ts.tz_localize("UTC") if ts.tzinfo is None else ts
                        if bar_dt < t_start - timedelta(days=2):
                            continue
                        if bar_dt > t_end + timedelta(days=2):
                            break
                        spy_points.append({
                            "ts": bar_dt.isoformat(),
                            "spy_pct_from_start": round((v / baseline - 1) * 100.0, 4),
                        })
        except Exception:
            spy_points = []  # silently degrade — chart still renders fleet line
    ...
    return JSONResponse({
        ...
        "points": points,
        # SPY benchmark — daily bars over the same window. Empty array if fetch failed.
        "spy_points": spy_points,
        "spy_current_pct": spy_points[-1]["spy_pct_from_start"] if spy_points else None,
    })
...
async def api_strategy_performance():
    ...
    return JSONResponse({
        ...
        # Removed 2026-04-29: the "40-75% expected annual" hardcoded label
        # biased decisions toward optimism without evidence. Replaced with an
        # honest disabled state until 30+ valid trades on a top strategy
        # provides a real basis for projection.
        "expected_annual": None,
        "expected_annual_source": "disabled_until_evidence",
        "timestamp": datetime.now(timezone.utc).isoformat(),
    })
